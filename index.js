...
app.use(cors({
  origin:[
   'http://localhost:5173',
   'https://course-wire.surge.sh',
    'https://course-project-auth-726d2.firebaseapp.com',
    'https://course-project-auth-726d2.web.app'
  ],
  credentials:true
}));
app.use(cors());
...
async function run() {
  try {
    ...
    const courseEnrollment=client.db("Enrollment").collection("manageEnrollment")
    ...
    app.get('/manageEnrollment',async(req,res)=>{
        const cursor=courseEnrollment.find();
        const result=await cursor.toArray()
        res.send(result)
       
      })

      app.post('/manageEnrollment',async(req,res)=>{
        const enrollment=req.body;
        console.log(enrollment)
        const result=await courseEnrollment.insertOne(enrollment)
